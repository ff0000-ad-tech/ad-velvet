...
export function init(slugs, dateSettings = {}) {
	...
	const addataSlug = getQueryParam('addata')
	if (addataSlug) {
...
export function isPreviewLocation() {
	...
	if (getQueryParam('velvet') === 'production') return false
	// local locations
	if (
		href.match(/^file/) ||
