...
/**
 * Converts a Velvet object like:
 *
 *  myKey: {
 *    type: "VelvetPrimitive",
 *    value: { ...some-data }
 *  }
 *
 ...
 *
 */
export const flatten = (velvetObj) => {
	if (!velvetObj) {
		return
	}
	const val = velvetObj.value
	// no value, then velvetObj is not a velvet-object
	if (val === undefined) {
		return velvetObj
	}
	switch (typeof val) {
		...
		case 'object':
			if (Array.isArray(val)) {
				return val.map((item) =>
					Object.entries(item).reduce((cacc, [k, v]) => {
						return {
							...cacc,
							[k]: parseVelvetData(v),
						}
