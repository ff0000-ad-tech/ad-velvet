...
let _splits
let _isPathed
let _allowDefault = true // defaults true
let _convertBreaks = false // defaults false
...
/** 
	...
	@param {object|string|boolean} arguments
		...
		<br>
		3. An object as the last argument (second for option 1, third for option 2), will provide options:
		- allowDefault: used with 'MultiSizeText' & 'MultiSizeImage', set to false to not fall back to default value. 
		- convertBreaks: see convertBreaks()
		- recursive: if true, will have the method search recurrsively through the data object for the key. Otherwise, it will only look at the level provided.
		<br>
		See the example.
	@description
		This method is used to get any node from the JSON by name. All nodes in Velvet have a 'type' and 'value'; the return is always the 'value' or end of the line key
	@example
		...
		var subtitle = Velvet.get('sub_title', { convertBreaks: true }) // This is <br>some text

		// Get a collection, such as a team for later use:
		var awayTeam = Velvet.get('away_team', { recursive: true }) 
		...
		var playerImgAway = Velvet.get(awayTeam, 'player_img', { allowDefault: false })
		// In a 728x90:
		// returns null
*/
export default function get() {
	...
	_recursive = !!(optRaw === true ? optRaw : optObj['recursive'])
	if (_debug)
		console.log('_allowDefault:', _allowDefault, '| _convertBreaks:', _convertBreaks, '| _recursive:', _recursive)
...
function readPropertyPath(_splits, obj) {
	...
	for (var i = 0; _result && (s = _splits[i]); i++) {
		...
		// check if the resultVal exists first
		_result = resultVal && s in resultVal ? resultVal[s] : undefined
	}
	return _result
}
