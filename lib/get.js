...
function readPropertyPath(_splits, obj) {
	...
	for (var i = 0; i < _splits.length; i++) {
		...
		const sIsIndex = !isNaN(parseInt(s))
		if (sIsIndex) {
			/** NOTE: _splits could be an index, like "0", which will evaluate false
			 * 	which causes the unexpected to happen here.
			 * 	So we're just going to let the loop proceed to the next split
			 */
			// _result = resultVal ? resultVal[s] : _result
		} else {
