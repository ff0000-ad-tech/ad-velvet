...
import * as VelvetUrls from './VelvetUrls.js'
import * as VelvetEvents from './VelvetEvents.js'
...
export function init(slugs, dateSettings = {}) {
	...
	if (isPreviewLocation()) {
		...
		_baseUrlNow = isQA ? VelvetUrls.previewQA : VelvetUrls.preview
	} else {
		console.log('\tLive requested')
		_baseUrlNow = isQa ? VelvetUrls.liveQA : VelvetUrls.live
	}
...
export function get() {
	...
	var source = _useRawObject ? _adDataRaw : arguments[0]
...
function handleSegmentLoadComplete(event) {
	...
	if (_settings.addata === 'USE_STATIC') {
		EventController.dispatch(VelvetEvents.STATIC)
		return
	}
...
function handleAdDataLoadComplete(event) {
	...
	_adDataRaw = JSON.parse(event.dataRaw)
	console.log('_adDataRaw:\n', _adDataRaw)

	_resolved()
	EventController.dispatch(VelvetEvents.INIT)
}

function handleFail() {
	EventController.dispatch(VelvetEvents.FAIL)
}
